...
if ($op = $_GET["op"]) {
	if ($op == "delete") {
		...
		foreach ($ids as $_id) {
			$_id = intval($_id);
			if ($_id > 0) {
				del_data($db, $table, $_id, 1, "删除QQ来源“{name}”") ? $del_ok++ : $del_fail++;
			}
		}
		if ($del_fail > 0) {
...
function add(hid) {
	set_high_light('');
	parent.load_src(1,'qq_from_edit.php?hid='+hid, 700, 300);
	return false;
}

function edit(id, obj) {
	set_high_light(obj);
	parent.load_src(1,'qq_from_edit.php?id='+id, 700, 300);
	return false;
}
</script>
</head>
...
<table class="headers" width="100%">
	<tr>
		<td class="headers_title" style="width:280px;"><nobr class="tips">QQ来源</nobr></td>
