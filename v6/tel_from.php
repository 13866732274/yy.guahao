...
if ($op = $_GET["op"]) {
	if ($op == "delete") {
		...
		foreach ($ids as $_id) {
			$_id = intval($_id);
			if ($_id > 0) {
				del_data($db, $table, $_id, 1, "删除电话来源“{name}”") ? $del_ok++ : $del_fail++;
			}
		}
		if ($del_fail > 0) {
...
function add(hid) {
	set_high_light('');
	parent.load_src(1,'tel_from_edit.php?hid='+hid, 700, 300);
	return false;
}

function edit(id, obj) {
	set_high_light(obj);
	parent.load_src(1,'tel_from_edit.php?id='+id, 700, 300);
	return false;
}
</script>
</head>
...
<table class="headers" width="100%">
	<tr>
		<td class="headers_title" style="width:280px;"><nobr class="tips">电话来源</nobr></td>
		<td class="header_cneter" align="center">
<?php
if (check_power("i", $pinfo, $pagepower)) {
	echo '<button onclick="add(0);return false;" class="button">添加</button>';
}
?>
		</td>
		<td class="headers_oprate" style="width:280px;"><form name="topform" method="GET"><nobr>关键词：<input name="searchword" value="<?php echo $_GET["searchword"]; ?>" class="input" size="15">&nbsp;<input type="submit" class="search" value="搜索" style="font-weight:bold" title="点击搜索">&nbsp;<button onclick="location='?'" class="search" title="退出条件查询">重置</button></nobr></form></td>
	</tr>
</table>
<!-- 头部 end -->
