...
if ($op = $_GET["op"]) {
	if ($op == "delete") {
		...
		foreach ($ids as $_id) {
			$_id = intval($_id);
			if ($_id > 0) {
				del_data($db, $table, $_id, 1, "删除医生“{name}”") ? $del_ok++ : $del_fail++;
			}
		}
		if ($del_fail > 0) {
...
$aTdFormat = array(
	...
	2=>array("title"=>"医生名字", "align"=>"left", "sort"=>"binary name", "defaultorder"=>1),
	1=>array("title"=>"编号", "align"=>"left", "sort"=>"binary name", "defaultorder"=>1),
	5=>array("title"=>"接诊人数限制(工作日/周末)", "width"=>"", "align"=>"left"),
	6=>array("title"=>"医生简介", "width"=>"", "align"=>"left"),
	3=>array("title"=>"添加时间", "align"=>"center", "sort"=>"addtime", "defaultorder"=>2),
	4=>array("title"=>"操作", "align"=>"center"),
);
...
function add() {
	set_high_light('');
	parent.load_src(1,'doctor_edit.php', 700, 400);
	return false;
}

function edit(id, obj) {
	set_high_light(obj);
	parent.load_src(1,'doctor_edit.php?id='+id, 700, 400);
	return false;
}
</script>
</head>
...
<table class="headers" width="100%">
	<tr>
		<td class="headers_title" style="width:280px;"><nobr class="tips"><?php echo $hospital_id_name[$user_hospital_id]; ?> - 医生列表</nobr></td>
...
?>
	<tr<?php echo $hide_line ? " class='hide'" : ""; ?>>
		...
		<td align="left" class="item"><?php echo $line["max_weekday"]."/".$line["max_weekend"]; ?></td>
		<td align="left" class="item"><?php echo $line["intro"]; ?></td>
